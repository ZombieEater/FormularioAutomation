...
from includes.funciones import escribir,mi_click,nuevo_driver,navegar,maximiza, selecciona, esperar, log_evento, leer_csv, completa_form, printKaos
...
print(f'Modo verborrágigo: {config.PRINT}')
print(f'Modo logeo compulsivo: {config.LOG}')
print('-----------------------------------------------')
...
for i, registro in enumerate(listado):
    try:
        ...
        btn_resultado = selecciona (By.CSS_SELECTOR, Locator.boton_resultado)

        assert btn_resultado.text == Messages.resultado_ok, Messages.mensaje_error

        if btn_resultado.text == Messages.resultado_ok:
            printKaos('-----' + Messages.mensaje_ok +'-----')
            log_evento('event',f'Procesado registro {registro['id']}.')
        else:
            printKaos('-----'+ Messages.mensaje_error +'-----')
            log_evento('error',f'Procesado registro {registro['id']}.')
